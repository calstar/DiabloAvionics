...
void mainloop(BoardState myBoardstate, int boardID){
    while(true){
        ...
        sendHeartbeat(boardID, myBoardState, SERVER_ADDRESS); //send heartbeats including board ID    

        if(heartbeatTimedOut(6000, 0)){ //example time values
           connectionLoss(myBoardState, boardID);
        }
        if(abortClearPacket() == true && necessaryForAbort == true){ //check if abort clear packet is received and necessary conditions for abort are met
            provideAbortData(myBoardstate, boardID, IPAddress, SERVER_ADDRESS); //send to server
        }
    }
}
void connectionLoss(BoardState myBoardstate, int boardID){
