...
ADS126X::ADC1Reading ADS126X::readADC1(){
  digitalWrite(cs_pin, LOW);
  ...
  ADS126X::ADC1Reading result;
  result.value = static_cast<int32_t>(ADC_BYTES.reg);
  result.checksumValid = checksumValid;
  return result;
}

ADS126X::ADC1Reading ADS126X::readADC1(uint8_t pos_pin, uint8_t neg_pin) {
...
bool ADS126X::check_checksum(uint32_t val, uint8_t checksum) {
  ...
  return ((sum ^ checksum) == 0); // XOR return 0 if no corruption
}
